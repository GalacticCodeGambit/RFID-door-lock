...
void handleRoot() {
  ...
  if (strlen(ssidnew) != 0) {
    message += "<input type='text' name='ssid' placeholder='" + String(ssidnew) + "'>";
  } else {
  ...
  if (strlen(psknew) != 0) {
    message += "<input type='text' name='psk' placeholder='" + String(psknew) + "'>";
  } else {
...
void handleStore3() { //storing SSID
  varcoppy = server.arg("ssid");
  ssidnew = varcoppy.data();
...
void handleStore4() { //storing Passkey
  varcoppy = server.arg("psk");
  psknew = varcoppy.data();
